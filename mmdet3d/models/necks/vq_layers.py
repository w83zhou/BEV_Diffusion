...
class VQEncoder(nn.Module):
    def __init__(
        ...
        # Add PatchMerging layer
        self.patch_merging = PatchMerging((self.h, self.w), dim=embed_dim, norm_layer=norm_layer)
...
#Patch upsample layer for the swin transformer
class PatchUpsample(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.deconv = nn.ConvTranspose2d(dim, dim // 4, kernel_size=2, stride=2, bias=False)
        self.linear = nn.Linear(dim // 4, dim // 2, bias=False)
        ...
        def forward(self, x, H, W):
            ...
            # Reshape x from (B, L, C) to (B, C, H, W)
            x = x.view(B, H, W, C).permute(0, 3, 1, 2)  # B, C, H, W

            # Apply deconvolution, shape becomes (B, C//4, H*2, W*2)
            x = self.deconv(x)  # B, C//4, H*2, W*2

            # Reshape x from (B, C//4, H*2, W*2) to (B, H*2*W*2, C//4)
            x = x.permute(0, 2, 3, 1).contiguous().view(B, -1, C // 4)  # B, H*2*W*2, C//4

            # Apply normalization, shape remains (B, H*2*W*2, C//4)
            x = self.norm(x)

            # Apply linear transformation, shape becomes (B, H*2*W*2, C//2)
            x = self.linear(x)  # B, H*2*W*2, C//2
...
class VQDecoder(nn.Module):
    def __init__(
        ...
        self.patch_upsample = PatchUpsample(embed_dim)
        self.blocks = [
            BasicLayer(
            embed_dim,
            ...
            num_heads=num_heads,
            window_size=8,
            #downsample=self.patch_upsample,
            downsample=None,
            ),
            BasicLayer(
            embed_dim,
    ...
    def forward(self, x):
        ...
        # add pos embed
        x = x + self.pos_embed # x: (4, 256, 512)
